...
| `extra assets` | no | none — see §5 when the user supplies one |
...
in; never let it silently override a non-negotiable in §3 — if it collides, say so and
...
- **One content row per canvas.** Title block + one row + closing line. A CTA does not fit
  alongside that — that is a two-slide carousel. ("Too much bloated" was the verdict on
  the dense version.) Split rather than compress.
...
3. **Pick the recipe** and read it before writing any HTML:
   ...
   - shared element map, applies to all → `references/layout-map.md`
4. **Copy the template** from `templates/` into the output folder and fill it. The
   templates carry every fixed value already; you change the tokens, the words, the
   icons and the background linework.
...
6. **Render and look at it.** `python3 scripts/render.py <file.html>` then **Read the PNG**
   and crop in at full resolution to check type and edges. Never report done unrendered.
7. **Check against §3 and `references/gotchas.md`** before handing over.
...
`NN` is zero-padded and counts within that date folder. Templates are blank references —
never publish one directly.
...
| File | Read it when |
|---|---|
| `references/layout-map.md` | always — the fixed element map and type scale |
